...
from app.schemas.schemas import Token, UserResponse, GitHubAuthRequest

router = APIRouter()


@router.post("/github", response_model=Token)
async def github_auth(auth_request: GitHubAuthRequest, db: Session = Depends(get_db)):
    """
    ...
    github_id = github_user["id"]  # Already an integer from GitHub API
...
@router.get("/check")
async def check_auth(current_user: Optional[User] = Depends(get_optional_current_user)):
    """
